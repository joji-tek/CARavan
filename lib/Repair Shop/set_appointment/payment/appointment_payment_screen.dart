...
import 'package:user1_bookingrepair/Repair%20Shop/set_appointment/invoice_appointment.dart';
import 'package:user1_bookingrepair/Repair%20Shop/set_appointment/payment/appointment_epayment.screen.dart';

class AppointmentPaymentScreen extends StatefulWidget {
  final String issuePrice; 
   // Price from CarRepairBookingScreen

  const AppointmentPaymentScreen({Key? key, required this.issuePrice, }) : super(key: key);
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            onPressed: () {
              int transactionStatus = 0; // Default transaction status

              if (_selectedPaymentMethod == 'Cash') {
                transactionStatus = 1; // Set status to 1 for Cash
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => LatestAppointmentScreen( ),
                  ),
                );
                  ...
                  builder: (BuildContext context) {
                    return AlertDialog(
                      ...
                      actions: [
                        TextButton(
                          onPressed: () {
                            transactionStatus = 2; // Set status to 2 for E-Payment
                            Navigator.pop(context); // Close dialog
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => AppointmentEpaymentScreen(
                                  totalAmount: totalAmount,
                                  transactionStatus: transactionStatus, 
                                  bookingId: bookingId, 
                                  isTransactionSuccessful: true, // Pass status to E-payment screen
                                ),
                              ),
                            );
                          },
